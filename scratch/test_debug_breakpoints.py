...
class TestBreakpointAwareDebugRun(unittest.TestCase):
    def test_run_stops_before_selected_source_line_and_manual_step_crosses_it(self):
        session = DebugSession(load_solution(), LEVEL_PATH, source_base_dir=ROOT_DIR)
        stopped = session.run(max_cycles=50, breakpoint_lines=[5], breakpoint_robot_id=0)
        ...
        self.assertEqual(stopped['execution']['breakpoint'], {
            'kind': 'line', 'robot_id': 0, 'pc': 2, 'line_num': 5, 'cycle': 2,
        })
        ...
        finished = session.run(max_cycles=50, breakpoint_lines=[5], breakpoint_robot_id=0)
    ...
    def test_cycle_zero_breakpoint_is_ignored_like_existing_web_run_semantics(self):
        session = DebugSession(load_solution(), LEVEL_PATH, source_base_dir=ROOT_DIR)
        result = session.run(max_cycles=1, breakpoint_lines=[3], breakpoint_robot_id=0)
    ...
    def test_conditional_breakpoint_uses_safe_authoritative_state(self):
        session = DebugSession('MOV 1 R0\nINC R0\nINC R0\nHLT', LEVEL_PATH, source_base_dir=ROOT_DIR)
        stopped = session.run(max_cycles=20, conditional_breakpoints=[{
            'line_num': 3,
    ...
    def test_register_and_ram_watchpoints_stop_after_mutation_and_rewind(self):
        register_session = DebugSession('MOV 4 R0\nHLT', LEVEL_PATH, source_base_dir=ROOT_DIR)
        ...
        self.assertEqual(changed['execution']['watchpoint'], {
            'kind': 'register', 'robot_id': 0, 'name': 'R0',
            'old_exists': True, 'old_value': 0,
            'new_exists': True, 'new_value': 4, 'cycle': 1,
        })
        ...
        stored = ram_session.run(max_cycles=10, watchpoints=[{'kind': 'ram', 'address': 0}])
    ...
    def test_debug_conditions_reject_code_execution_and_invalid_watchpoints(self):
        for source in ('__import__("os")', 'R0.__class__', '[R0][0]', 'open(1)', '"text" == "text"'):
    ...
    def test_breakpoint_validation_rejects_ambiguous_or_invalid_inputs(self):
        session = DebugSession('NOP\nHLT', LEVEL_PATH, source_base_dir=ROOT_DIR)
        invalid_cases = [
            {'breakpoint_lines': '2'}, {'breakpoint_lines': [0]},
            {'breakpoint_lines': [True]}, {'breakpoint_lines': [1.5]},
            {'breakpoint_lines': {'lines': [], 'unknown': []}},
            {'breakpoint_robot_id': True}, {'breakpoint_robot_id': 1},
        ]
...
class TestBreakpointAwareHTTPRun(unittest.TestCase):
    ...
    def create_session(self, code=None):
        status, payload = self.request_json('POST', '/api/debug/sessions', {
            'level': 'level1.json',
            'code': load_solution() if code is None else code,
            'optimize': False,
        })
    ...
    def test_http_run_stops_at_breakpoint_and_preserves_rewind_history(self):
        ...
        try:
            status, stopped = self.request_json('POST', f'/api/debug/sessions/{session_id}/run', {
                'max_cycles': 32, 'breakpoint_lines': [5], 'breakpoint_robot_id': 0,
            })
    ...
    def test_http_structured_stop_spec_supports_conditionals_and_watchpoints(self):
        conditional_id = self.create_session()
        try:
            status, stopped = self.request_json('POST', f'/api/debug/sessions/{conditional_id}/run', {
                'max_cycles': 32,
            ...
            self.assertEqual(status, 200)
            self.assertEqual(stopped['cycles'], 2)
            self.assertEqual(stopped['execution']['breakpoint']['kind'], 'conditional')
        finally:
            self.request_json('DELETE', f'/api/debug/sessions/{conditional_id}')

        watch_id = self.create_session('MOV 0 R0\nMOV 7 R1\nSTORE R1 R0\nHLT')
        try:
            status, stopped = self.request_json('POST', f'/api/debug/sessions/{watch_id}/run', {
                'max_cycles': 32,
                'breakpoint_robot_id': 0,
                'breakpoint_lines': {
                    'lines': [],
            ...
            self.assertEqual(status, 200)
            self.assertEqual(stopped['cycles'], 3)
            self.assertTrue(stopped['execution']['stopped_by_watchpoint'])
            self.assertEqual(stopped['execution']['watchpoint']['new_value'], 7)
        finally:
            self.request_json('DELETE', f'/api/debug/sessions/{watch_id}')
    ...
    def test_http_run_rejects_bad_breakpoint_payload(self):
        ...
        try:
            ...
            status, payload = self.request_json(
                'POST', f'/api/debug/sessions/{session_id}/run',
                {'max_cycles': 4, 'breakpoint_lines': {'lines': [], 'mystery': []}},
            )
            self.assertEqual(status, 400)
